...
- (void)backgroundVideoAndStickerImage:(NSData *)backgroundVideo stickerImage:(NSData *)stickerImage attributionURL:(NSString *)attributionURL
{
    // Verify app can open custom URL scheme. If able,
    // assign assets to pasteboard, open scheme.
    NSURL *urlScheme = [NSURL URLWithString:@"instagram-stories://share"];
    ...
    if (method) {
        if([method isEqualToString:@"shareBackgroundImage"]) {
            ...
            if (backgroundVideoURL == nil || stickerURL == nil) {
                ...
                [self backgroundVideoAndStickerImage:backgroundVideo stickerImage:stickerImage attributionURL:attrURL];
            }
        }
    } else {
